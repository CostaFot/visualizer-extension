...
- `Commands/PlayTestSignalCommand.cs` + `Audio/TestSignal.cs` — the built-in self-test (#11):
...
same signal also ships in-app as the "Test visualizer" command (#11, `Audio/TestSignal.cs` —
...
the built-in self-test (#11 — runtime-synthesized tone-ladder + sweep played via WinRT
`MediaPlayer`, surfaced in the hub and every band's right-click menu). The battery-aware throttle
(#8) was CANCELLED 2026-08-16 — don't re-propose it.
